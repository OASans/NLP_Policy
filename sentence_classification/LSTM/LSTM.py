...
train_iter, val_iter, test_iter = data.Iterator.splits(
            ...
            batch_sizes=(batch_size, len(val), len(test)), # 训练集设置batch_size,验证集整个集合用于测试
    )
...
class BiLSTM_Attention(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_hiddens, num_layers):
        ...
        self.decoder = nn.Linear(2*num_hiddens, 21)  # here
...
def train(train_iter,val_iter, test_iter, net, loss, optimizer, num_epochs):
    batch_count = 0
    for epoch in range(num_epochs):
        ...
        print(
            'epoch %d, loss %.4f, train acc %.3f, val acc %.3f, test acc %.3f, time %.1f sec'
            % (epoch + 1, train_l_sum / batch_count, train_acc_sum / n,
               val_acc, test_acc, time.time() - start))


lr, num_epochs = 0.001, 40  # here
optimizer = torch.optim.Adam(net.parameters(), lr=lr)
loss = nn.CrossEntropyLoss()
train(train_iter, val_iter, test_iter, net, loss, optimizer, num_epochs)
